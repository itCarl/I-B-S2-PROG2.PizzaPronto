...
import de.thb.dim.pizzaPronto.buisnessObjects.io.MenuImporter;
...
public class MenuVO {
	...
	private static ArrayList<DishVO> dishes;
	 ...
	 * Constructos
	 */
	public MenuVO() throws IOException {
		this.initMenu();
	}
	...
	private void initMenu() throws IOException {
		...
		MenuImporter menu = new MenuImporter();
		String f = "myLittleMenu.txt";
		
		if(menu.readMenu(f) != null) {

			LinkedList<DishVO> m = menu.readMenu(f);
			MenuVO.dishes = new ArrayList<DishVO>(m);
			
		} else {
			
			MenuVO.dishes = new ArrayList<DishVO>();
...
//			dishes[17] = new DessertVO(23, "Hausgemachtes Tiramisu", 2.80f);
	...
	public String toString() {
		...
		i = 0;
		do {
			if (MenuVO.dishes.get(i) != null) {
				sb.append(MenuVO.dishes.get(i).getNumber() + "\t");
				sb.append(MenuVO.dishes.get(i).getName() + "\t");
				sb.append(MenuVO.dishes.get(i).ingredientsToString());
				sb.append("\n\tPrice:\t\t\t" + dFormat.format(MenuVO.dishes.get(i).getPrice()) + " Euro");
				if(MenuVO.dishes.get(i).getNumber() == MenuVO.dishes.get(i+1).getNumber()){
					sb.append("\n\tPrice:\t\t\t" + dFormat.format(MenuVO.dishes.get(i+1).getPrice()) + " Euro");
					sb.append("\n");
					i+=2;
			} else
				i++;
			}
		} while (i < MenuVO.dishes.size()); // && this.dishes.get[i] instanceof PizzaVO 
	...
	public DishVO getDish(int index) {
		return MenuVO.dishes.get(index);
	}
